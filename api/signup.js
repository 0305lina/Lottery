...
function mapSupabaseError(error) {
  if (error.code === '23505') {
    return { status: 409, error: '이미 가입된 이메일입니다.' };
  }
  if (error.code === 'PGRST205') {
    return { status: 500, error: '가입 테이블이 없습니다. Supabase에서 schema.sql을 실행해 주세요.' };
  }
  return { status: 500, error: '가입 정보 저장 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.' };
}
